/-
Vendored (COPIED, not `require`d) to keep Mathlib as the sole dependency.
Source: the author's prior Lean development on KL divergence (author: Dhruv Gupta, Apache-2.0).
Copied verbatim 2026-06-01. Only structural edit allowed: namespace/imports to compile against Mathlib alone.
No edit was necessary: the original import block is already pure `Mathlib.*`
(`Mathlib.Analysis.SpecialFunctions.Log.Basic`, `Mathlib.Algebra.BigOperators.Group.Finset.Basic`).
Declarations copied: FinitePMF, klDivFinitePMF, crossEntropyFinitePMF, expectFinitePMF, HasPositivePrior.
NOTE: the source file contains NO Gibbs / nonnegativity / klDivFinitePMF_nonneg lemma; such a lemma
must be proved from Mathlib; it is proved in EntropyFloor.lean (klDivFinitePMF_nonneg).
-/
/-
